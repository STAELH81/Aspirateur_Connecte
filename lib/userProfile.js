...
function buildUserProfileEmbed(user, member, viewerMember) {
  ...
  const quest = gamblingProgress.getQuestStatus(user.id);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "Quete du jour",
        value: `${quest.label}\n${gamblingProgress.questProgressField(quest)} · +**${quest.reward}** coins`,
        inline: false,
      }
    );
  ...
  embed.setFooter({ text: "Les Girlsss · profil" });
  return embed;
}
