...
function tableDisplayName(member, userId) {
  ...
  if (visualWidth(display) <= 16) return display;
  const username = member?.user?.username;
  if (username && visualWidth(username) <= 16) return username;
  return truncateVisual(display, 16);
}

function buildTableBlock(rows) {
  const nameCol = 16;
  const header = `${padVisual("Nom", nameCol)}  Q  C`;
  const body = rows.map((r) => `${padVisual(r.name, nameCol)}  ${r.quest}  ${r.coop}`);
...
async function buildEmbeds(rows, guild) {
  ...
  const pulse = `Aujourd'hui : quêtes **${totals.quest.g}**🟢 · coop **${recap.totalGames}/${coopGoal.GOAL_GAMES}**`;
  const legend = [
    "**Quête** — 🟢 réclamée · 🟠 faite / à claim · 🔴 pas faite",
    "**Coop** — 🟢 réclamée · 🟠 en cours / à claim · 🔴 pas commencé · ⚫ trop tard",
    "",
    pulse,
    streakLine,
    "",
    `**Quetes du jour** · **Coop du jour** · **Infos quotidiennes** · **Refresh** · **${SITE_URL}**`,
  ]
    .filter(Boolean)
    .join("\n");
  ...
  const baseEmbed = () =>
    new EmbedBuilder()
      ...
      .addFields(
        {
          name: "Quête du jour",
          value: `**${quest.label}**\n+**${quest.reward}** coins · serie jusqu'a **+21** bonus`,
          inline: true,
        },
        {
          name: "Coop serveur",
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    if (i === 0) {
      embed.setFooter({
        text: `Maj auto ~${BOARD_REFRESH_MS / 1000}s · staff : /quests panel`,
      });
    }
    embeds.push(embed);
  }
