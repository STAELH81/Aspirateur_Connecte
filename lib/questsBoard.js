...
/** Noms pleine largeur + 3 cols pastilles (Q | C | Daily Work). */
function buildMemberTableFields(rows) {
  const namesValue = rows.map((r) => r.name).join("\n");
  const dailyWork = rows.map((r) => `${r.daily} ${r.work}`).join("\n");
  ...
  return [
    ...
    { name: "Daily · Work", value: dailyWork.slice(0, 1024), inline: true },
  ];
}
