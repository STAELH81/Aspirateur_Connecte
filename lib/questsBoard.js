...
const COLUMN_NAME_MAX = 18;

/** Pseudo serveur → ASCII lisible. */
function tableDisplayName(member, userId) {
  const display = member?.displayName || member?.user?.username || userId.slice(0, 8);
  return truncateNameJs(display, COLUMN_NAME_MAX);
}

const DOT_COL_W = 5;
const DOT_DISPLAY_W = { "🟢": 2, "🟠": 2, "🔴": 2, "⚫": 1 };

function dotDisplayWidth(str) {
  const e = String(str ?? "?");
  if (Object.prototype.hasOwnProperty.call(DOT_DISPLAY_W, e)) return DOT_DISPLAY_W[e];
  if (e.length >= 2 && e.charCodeAt(0) >= 0xd800 && e.charCodeAt(0) <= 0xdbff) return 2;
  return 1;
}

function dotCell(emoji) {
  const e = String(emoji ?? "?");
  const pad = DOT_COL_W - dotDisplayWidth(e);
  return e + " ".repeat(Math.max(0, pad));
}

function statLine(row) {
  return dotCell(row.quest) + dotCell(row.coop) + dotCell(row.daily) + dotCell(row.work);
}

/** Deux colonnes inline Discord : noms | pastilles (blocs code courts = pas de wrap). */
function buildMemberTableFields(rows) {
  const nameLines = rows.map((r) => r.name);
  const headerLine = dotCell("Q") + dotCell("C") + dotCell("D") + dotCell("W");
  const statLines = rows.map(statLine);

  const namesValue = ["```", ...nameLines, "```"].join("\n");
  const statsValue = ["```", headerLine, ...statLines, "```"].join("\n");
