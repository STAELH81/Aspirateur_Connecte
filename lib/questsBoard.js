...
function tableDisplayName(member, userId) {
  ...
  if (/^[\x20-\x7E]+$/.test(display) && display.length <= 16) return display;
  if (/^[\x20-\x7E]+$/.test(username) && username.length <= 16) return username;
...
function buildTableBlock(rows) {
  const nameCol = 16;
  const header = `${padTableName("Nom", nameCol)}  Q  C`;
  const body = rows.map((r) => `${padTableName(r.name, nameCol)}  ${r.quest}  ${r.coop}`);
