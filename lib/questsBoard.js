...
function tableDisplayName(member, userId) {
  ...
  if (/^[\x20-\x7E]+$/.test(display) && display.length <= TABLE_NAME_COL) return display;
  if (/^[\x20-\x7E]+$/.test(username) && username.length <= TABLE_NAME_COL) return username;
  return username.slice(0, TABLE_NAME_COL);
}
...
function buildTableBlock(rows) {
  const headerDots = TABLE_STATUS_HEADERS.map((h) => padVisual(h, TABLE_STATUS_COL_W)).join("");
  const header = `${padTableName("Nom", TABLE_NAME_COL)}${headerDots}`;
