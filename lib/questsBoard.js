...
function graphemeWidth(g) {
  ...
  // Pseudos stylés Discord (fraktur, bold math…) — ~2 cellules en bloc code
  if (cp >= 0x1d400 && cp <= 0x1d7ff) return 2;
  if (
    (cp >= 0x1100 && cp <= 0x115f) ||
    (cp >= 0x2e80 && cp <= 0xa4cf) ||
    (cp >= 0xac00 && cp <= 0xd7a3) ||
...
const TABLE_NAME_COL = 18;
const TABLE_STATUS_COL_W = 6;
const TABLE_STATUS_HEADERS = ["Quete", "Coop", "Daily", "Work"];

/** Pseudo serveur (displayName) — Unicode / fonts stylées via padVisual. */
function tableDisplayName(member, userId) {
  const display = member?.displayName || member?.user?.username || userId.slice(0, 8);
  return truncateVisual(display, TABLE_NAME_COL);
}

function padTableName(str, width) {
  return padVisual(String(str ?? ""), width);
}

/** Colonnes espacées : Quete · Coop · Daily · Work (gap extra entre Coop et Daily). */
function formatStatusRow(values) {
  const cells = values.map((v) => padVisual(v, TABLE_STATUS_COL_W));
  if (cells.length < 4) return cells.join(" ");
  return `${cells[0]} ${cells[1]}  ${cells[2]} ${cells[3]}`;
}

function buildTableBlock(rows) {
  const header = `${padTableName("Nom", TABLE_NAME_COL)}${formatStatusRow(TABLE_STATUS_HEADERS)}`;
  const body = rows.map(
    (r) => `${padTableName(r.name, TABLE_NAME_COL)}${formatStatusRow([r.quest, r.coop, r.daily, r.work])}`
  );
