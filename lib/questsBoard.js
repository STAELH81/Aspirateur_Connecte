...
function normalizeNameForTable(str) {
  ...
  for (const g of [...String(str ?? "")]) {
    out += mathAlphanumericToAscii(g) ?? g;
  }
  return out;
}
...
const DISCORD_CODE_LINE_MAX = 40;
const TABLE_STATUS_COL_W = 3;
const TABLE_NAME_COL =
  DISCORD_CODE_LINE_MAX - (4 * TABLE_STATUS_COL_W + 3); // 25 — max sans wrap Discord

/** Pseudo serveur → ASCII lisible pour alignement monospace (fonts Discord). */
function tableDisplayName(member, userId) {
  const display = member?.displayName || member?.user?.username || userId.slice(0, 8);
  return truncateVisual(normalizeNameForTable(display), TABLE_NAME_COL);
}

function padTableName(str, width) {
  return padVisual(normalizeNameForTable(String(str ?? "")), width);
}

/** Pastilles alignées (3 car. chacune) · une seule ligne ≤ 40 car. */
function formatStatusRow(values) {
  return values.map((v) => padVisual(v, TABLE_STATUS_COL_W)).join(" ");
}
