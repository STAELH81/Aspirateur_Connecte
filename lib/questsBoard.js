...
function normalizeNameForTable(str) {
  ...
  if (/[\u{1d400}-\u{1d7ff}]/u.test(out)) return "";
  return out || "";
}

function isReadableTableName(name) {
  return Boolean(name && name.length >= 2 && !/[^\x20-\x7E\u00C0-\u017F]/.test(name));
}

function truncatePlainName(name, maxLen) {
  const chars = [...String(name).trim()];
  if (chars.length <= maxLen) return chars.join("");
  return `${chars.slice(0, maxLen - 1).join("")}…`;
}

/** Colonne inline étroite (~14 car. sans retour à la ligne). */
const COLUMN_NAME_MAX = 14;
...
function tableDisplayName(member, userId) {
  ...
  const display = member?.displayName || globalName || username;

  const strippedLen = [...String(display).replace(/\s+/g, "")].length;
  let name = normalizeNameForTable(display);
  const decodedLen = name.replace(/\s/g, "").length;
  if (
    !isReadableTableName(name) ||
    (strippedLen > 0 && decodedLen < strippedLen * 0.75)
  ) {
    name = normalizeNameForTable(globalName) || normalizeNameForTable(username) || username;
  }
  return truncatePlainName(name, COLUMN_NAME_MAX);
}

/** Pastilles même famille Unicode (2 car. JS chacune) → colonnes alignées. */
const DOT_COL_W = 5;

function dotCell(emoji) {
  const e = String(emoji ?? "🟥");
  return e + " ".repeat(Math.max(0, DOT_COL_W - e.length));
}

function statLine(row) {
  return (
    dotCell(row.quest) + dotCell(row.coop) + dotCell(row.daily) + dotCell(row.work)
  );
}

/** Deux colonnes inline : noms (texte) | pastilles (bloc code). */
function buildMemberTableFields(rows) {
  const namesValue = rows.map((r) => r.name).join("\n");
  const statsValue = ["```", ...rows.map(statLine), "```"].join("\n");
