...
function tableDisplayName(member, userId) {
  ...
  return truncateNameJs(display, TABLE_NAME_MAX);
}

/** Liste texte (pas de bloc code — Discord coupe les ``` ~36 car./ligne). */
function buildMemberListBlock(rows) {
  return rows
    .map((r) => {
      const dots = [r.quest, r.coop, r.daily, r.work].join("  ");
      return `${r.name}  ·  ${dots}`;
    })
    .join("\n");
}
...
async function buildEmbeds(rows, guild) {
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    const listValue = buildMemberListBlock(chunk).slice(0, 1024);
    const embed =
      i === 0
        ? baseEmbed().addFields({
            name: "Suivi membres",
            value: `**Quete · Coop · Daily · Work**\n${listValue}`,
          })
        : new EmbedBuilder()
            .setColor(COLOR_UI)
