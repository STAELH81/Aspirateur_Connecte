...
function normalizeNameForTable(str) {
  ...
  for (const g of [...String(str ?? "")]) {
    const ascii = mathAlphanumericToAscii(g);
    if (ascii) {
      out += ascii;
      continue;
    }
...
function truncateNameJs(str, maxLen) {
  const name = normalizeNameForTable(str);
  if (name.length <= maxLen) return name;
  return `${name.slice(0, maxLen - 1)}…`;
}

/** Discord coupe les ``` des embeds ~36 car. JS (pas 40). */
const DISCORD_CODE_LINE_MAX = 36;
const TABLE_STATUS_GAP = "  ";
const TABLE_EMOJI_CELL = 2;
const TABLE_STATUS_JS_LEN = 4 * TABLE_EMOJI_CELL + 3 * TABLE_STATUS_GAP.length; // 14
const TABLE_NAME_JS = DISCORD_CODE_LINE_MAX - TABLE_STATUS_JS_LEN; // 22

function padEmojiCell(emoji) {
  return String(emoji ?? "?").padEnd(TABLE_EMOJI_CELL, " ");
}

function padNameJs(str, width = TABLE_NAME_JS) {
  return truncateNameJs(str, width).padEnd(width, " ");
}

function formatStatusRow(values) {
  return values.map(padEmojiCell).join(TABLE_STATUS_GAP);
}

function buildTableLine(row) {
  return padNameJs(row.name) + formatStatusRow([row.quest, row.coop, row.daily, row.work]);
}

/** Pseudo serveur → ASCII lisible (alignement monospace). */
function tableDisplayName(member, userId) {
  const display = member?.displayName || member?.user?.username || userId.slice(0, 8);
  return truncateNameJs(display, TABLE_NAME_JS);
}

function buildTableBlock(rows) {
  const body = rows.map((r) => buildTableLine(r));
  return ["```", ...body, "```"].join("\n");
}
...
async function buildEmbeds(rows, guild) {
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    const embed =
      i === 0
        ? baseEmbed().addFields({
            name: "Suivi membres · Quete · Coop · Daily · Work",
