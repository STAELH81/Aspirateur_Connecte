...
async function buildEmbeds(rows, guild) {
  ...
  const legend = [
    ...
    streakLine,
    "",
    `**Ma quete** = ton suivi perso · **Refresh** = maj tableau · **${SITE_URL}**`,
  ]
    .filter(Boolean)
    .join("\n");
...
async function handleQuestsPanelMyQuest(interaction) {
  ...
  const lines = [
    ...
    coop.claimed
      ...
      : coop.canClaim
        ? "Coop : **Claim coop** disponible."
        : coop.goalMet && !coop.eligible
          ? "Coop : objectif atteint avant ta 1re partie — pas eligible."
          : coop.goalMet
            ? "Coop : joue au casino puis **Claim coop**."
