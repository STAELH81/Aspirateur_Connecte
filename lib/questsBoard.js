...
const TABLE_NAME_COL = 14;
const TABLE_STATUS_COL_W = 5;
const TABLE_STATUS_HEADERS = ["Quete", "Coop", "Daily", "Work"];

/** Pseudo serveur (displayName) — comme avant, y compris Unicode / fonts stylées. */
...
function buildTableBlock(rows) {
  const header = TABLE_STATUS_HEADERS.reduce(
    (line, label) => line + padVisual(label, TABLE_STATUS_COL_W),
    padTableName("Nom", TABLE_NAME_COL)
  );
  const body = rows.map((r) => {
    const dots = [r.quest, r.coop, r.daily, r.work]
      .map((d) => padVisual(d, TABLE_STATUS_COL_W))
      .join("");
    return `${padTableName(r.name, TABLE_NAME_COL)}${dots}`;
  });
