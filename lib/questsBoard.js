...
function isPlainNick(str) {
  const s = String(str ?? "").trim();
  return s.length >= 1 && /^[\x20-\x7E\u00C0-\u017F]+$/.test(s);
}
...
function tableDisplayName(member, userId) {
  ...
  if (isPlainNick(display)) return truncatePlainName(display, COLUMN_NAME_MAX);
  if (isPlainNick(globalName)) return truncatePlainName(globalName, COLUMN_NAME_MAX);

  const decoded = normalizeNameForTable(display);
  if (isPlainNick(decoded) && decoded.length >= 3) {
    return truncatePlainName(decoded, COLUMN_NAME_MAX);
  }

  return truncatePlainName(username, COLUMN_NAME_MAX);
}

function statColumn(rows, key) {
  return rows.map((r) => r[key]).join("\n");
}

/** Noms pleine largeur + 4 colonnes pastilles (1 emoji/ligne, alignement garanti). */
function buildMemberTableFields(rows) {
  const namesValue = rows.map((r) => r.name).join("\n");

  return [
    { name: "Nom", value: namesValue.slice(0, 1024), inline: false },
    { name: "Quete", value: statColumn(rows, "quest").slice(0, 1024), inline: true },
    { name: "Coop", value: statColumn(rows, "coop").slice(0, 1024), inline: true },
    { name: "Daily", value: statColumn(rows, "daily").slice(0, 1024), inline: true },
    { name: "Work", value: statColumn(rows, "work").slice(0, 1024), inline: true },
  ];
}
