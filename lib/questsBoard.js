...
function checkQuestsChannel(interaction) {
  ...
  if (configured) {
    return {
      ok: false,
      message: `Utilise le panneau money dans <#${configured}>.`,
    };
  }
  return { ok: true };
}
...
function tableDisplayName(member, userId) {
  ...
  if (/^[\x20-\x7E]+$/.test(display) && display.length <= 12) return display;
  if (/^[\x20-\x7E]+$/.test(username) && username.length <= 12) return username;
...
function buildTableBlock(rows) {
  const nameCol = 12;
  const header = `${padTableName("Nom", nameCol)} daily work quest coop`;
  const body = rows.map(
    (r) =>
      `${padTableName(r.name, nameCol)}  ${r.daily}     ${r.work}     ${r.quest}     ${r.coop}`
  );
...
async function buildEmbeds(rows, guild) {
  ...
  const recap = gamblingProgress.getDailyRecap();
  const totals = countDots(rows);
  const streakLine = await streakPulseLine(guild);

  const pulse = `Coop **${recap.totalGames}/${coopGoal.GOAL_GAMES}** · quetes **${totals.quest.g}**🟢`;
  const legend = [
    "🟢 fait / claim · 🔴 pas encore · coop ⚫ trop tard",
    pulse,
    streakLine,
    "**aspirateurconnecte.netlify.app**",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  const baseEmbed = () =>
    new EmbedBuilder()
      ...
      .addFields(
        {
          name: "Quete du jour",
          value: `**${quest.label}** · +**${quest.reward}** · serie +21 max`,
          inline: true,
        },
        {
          name: "Coop",
          value: `**${recap.totalGames}/${coopGoal.GOAL_GAMES}** \`${coopBar}\` · +**${coopGoal.REWARD_COINS}**`,
          inline: true,
        },
        {
          name: "Maj",
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    const embed =
      i === 0
        ...
        : new EmbedBuilder()
            .setColor(COLOR_UI)
            .setTitle(`Money Center (suite ${Math.floor(i / chunkSize) + 1})`)
            .addFields({
              name: "Suivi membres",
              value: buildTableBlock(chunk).slice(0, 1024),
            });
    ...
    if (i === 0) {
      embed.setFooter({
        text: `Maj auto ~${BOARD_REFRESH_MS / 1000}s · msgs bleus ~5 min · /quests panel`,
      });
    }
    embeds.push(embed);
  }
...
async function handleQuestsPanelClaimCoop(interaction) {
  ...
  economyLog
    .logTx(interaction.client, {
      ...
      balanceBefore: before,
      balanceAfter: result.balance,
      details: `Bonus coop **+${result.reward}** coins`,
    })
    .catch(() => {});
