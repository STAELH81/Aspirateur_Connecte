...
function buildTableBlock(rows) {
  const nameCol = 11;
  const header = `${padTableName("Nom", nameCol)} quetes coop daily work`;
  const body = rows.map(
    (r) =>
      `${padTableName(r.name, nameCol)}  ${r.quest}     ${r.coop}     ${r.daily}     ${r.work}`
  );
...
async function buildEmbeds(rows, guild) {
  ...
  const legend = [
    ...
    "",
    "Pastilles : 🟢 fait · 🔴 pas fait · ⚫ coop trop tard",
    streakLine,
  ]
    .filter(Boolean)
    .join("\n");
