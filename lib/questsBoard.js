...
/** Colonne inline étroite : pas de bloc code (wrap ~13 car.). */
...
function tableDisplayName(member, userId) {
  ...
  if (!name) name = normalizeNameForTable(username) || username;
  return truncateNameJs(name, COLUMN_NAME_MAX);
}

/** Slots fixes par pastille (5 car. JS, 5 colonnes visuelles en monospace). */
const DOT_SLOT = {
  "🟢": "🟢   ",
  "🟠": "🟠  \u200b",
  "🔴": "🔴   ",
...
function dotSlot(emoji) {
  return DOT_SLOT[String(emoji ?? "?")] ?? "??   ";
}

function statLine(row) {
  return (
    dotSlot(row.quest) + dotSlot(row.coop) + dotSlot(row.daily) + dotSlot(row.work)
  );
}
