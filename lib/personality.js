...
function welcomeMessage(member, guildName, memberCount) {
  const lines = [
    ...
    "",
    "Prends tes roles dans les salons dedies — `/help` · `/money` pour les coins.",
  ];
    ...
    new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: "Economie — /money",
          value: [
            "`balance [membre]` — solde (100 coins au depart)",
            "`daily` — bonus toutes les 24h",
            "`work` — petit gain (cooldown 45 min)",
            "`pay` — envoyer des coins a quelqu'un",
        ...
        },
        {
          name: "Casino — /casino",
          value: [
            "`coinflip` — pile/face (mise min 10, max 25% du solde)",
            "`slots` — machine a sous (symboles rares = gros gain)",
          ].join("\n"),
        }
      ),
    new EmbedBuilder()
