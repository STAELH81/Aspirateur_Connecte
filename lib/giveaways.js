...
function buildEmbed(giveaway) {
  ...
  const embed = new EmbedBuilder()
    .setTitle(
      ended ? personality.giveaway.titleEnded : personality.giveaway.titleActive
    )
    .setColor(ended ? personality.COLOR_SUCCESS : personality.COLOR_GIVEAWAY)
    .addFields(
  ...
  if (!ended) {
    ...
    embed.setDescription("Personne — le vent souffle dans le vide.");
  }
  ...
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(personality.giveaway.buttonLabel)
...
async function endGiveaway(client, messageId, { reroll = false } = {}) {
  ...
  if (g.winners.length > 0) {
    ...
    const prize = sanitizePrize(g.prize);
    content = reroll
      ? personality.giveaway.reroll(winnersText, prize)
      : personality.giveaway.win(winnersText, prize);

    if (g.prizeRoleId || parseRoleIdFromPrize(g.prize)) {
      if (roleResult.error === "hierarchy") {
        content += `\n${personality.giveaway.roleHierarchy(roleResult.roleName || "VIP")}`;
      } else if (roleResult.error === "permission") {
        content += `\n${personality.giveaway.rolePermission}`;
      } else if (roleResult.error === "role") {
        content += `\n${personality.giveaway.roleMissing}`;
      } else if (roleResult.awarded.length > 0) {
        content += `\n${personality.giveaway.roleOk(roleResult.roleName)}`;
      } else if (roleResult.failed.length > 0) {
        content += `\n${personality.giveaway.roleFailed}`;
      }
    }
  } else if (!reroll) {
    content = personality.giveaway.noParticipants(sanitizePrize(g.prize));
  }
