...
function pushGitHubEnabled() {
  const flag = process.env.DASHBOARD_PUSH?.trim().toLowerCase();
...
function buildSnapshot() {
  ...
  return {
    ...
    coopGoal: {
      ...
      goalMet: recap.totalGames >= coopGoal.GOAL_GAMES,
    },
    todayCasino: {
      dayKey: recap.dayKey,
...
async function syncDashboard(options = {}) {
  ...
  if (!pushGitHubEnabled()) {
    return {
      ...
      github: {
        ok: false,
        reason: "Push GitHub desactive (DASHBOARD_PUSH=0). Fichiers locaux mis a jour seulement.",
      },
    };
  }
