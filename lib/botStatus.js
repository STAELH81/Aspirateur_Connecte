...
const ENV_SALONS_ROCK = [
  ...
  { key: "ECONOMY_LOG_CHANNEL_ID", label: "economy logs" },
  { key: "WELCOME_CHANNEL_ID", label: "welcome" },
  { key: "MUSIC_VOICE_CHANNEL_ID", label: "radio vocale", optional: true },
  { key: "SUGGESTIONS_CHANNEL_ID", label: "suggestions", optional: true },
];

function envSalonList() {
  return profile.isRockAndRoll() ? ENV_SALONS_ROCK : ENV_SALONS_GIRLSSS;
}
...
async function checkSalonChannels(guild) {
  const rows = [];
  for (const { key, label } of envSalonList()) {
...
async function checkBotPerms(guild) {
  ...
  if (profile.isRockAndRoll()) {
    need.push(["Connect (vocal)", PermissionFlagsBits.Connect]);
    need.push(["Speak (vocal)", PermissionFlagsBits.Speak]);
  }
...
async function buildStatusEmbed(client) {
  ...
  const salonEnvLines = envSalonList().map((e) => envLine(e));
  ...
  const statusTitle = profile.isRockAndRoll()
    ? `BotQuick — v${BOT_VERSION}`
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: `${profile.footerText()} · /botstatus` })
    .setTimestamp();
}
