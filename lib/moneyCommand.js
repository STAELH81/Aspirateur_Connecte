  ...
  return new SlashCommandBuilder()
    ...
    .addSubcommandGroup((group) => {
      group
        ...
        .addSubcommand((sub) =>
          sub.setName("panel").setDescription("Poster le panneau banque dans ce salon")
        )
        .addSubcommand((sub) =>
          sub.setName("infos-panel").setDescription("Poster le guide gambling")
        )
        .addSubcommand((sub) =>
          sub
        ...
        .addSubcommand((sub) =>
          sub
            .setName("fermer-parties")
            .setDescription("Ferme les parties casino ouvertes (blackjack, sans remboursement)")
        );

      if (profile.feature("shop")) {
        group
          .addSubcommand((sub) => sub.setName("shop-panel").setDescription("Poster le panneau shop"))
          .addSubcommand((sub) =>
            sub.setName("shop-stock-reset").setDescription("Reset manuel du stock shop persistant")
          );
      }
      return group;
    });
