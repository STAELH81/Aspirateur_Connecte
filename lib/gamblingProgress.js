...
function getUser(data, userId) {
  ...
  u.streakTierClaims = u.streakTierClaims || {};
  u.stats = u.stats || {
    casinoGames: 0,
    casinoWins: 0,
    casinoNet: 0,
...
function getStreakTierStatus(userId) {
  const streak = getQuestStreakInfo(userId);
  const data = ensureData();
  const user = getUser(data, userId);
  const claims = user.streakTierClaims || {};
  return STREAK_TIERS.map((tier) => ({
    days: tier.days,
    bonus: tier.bonus,
    reached: streak.streak >= tier.days,
    claimed: Boolean(claims[String(tier.days)]),
    canClaim: streak.streak >= tier.days && !claims[String(tier.days)],
  }));
}

function claimStreakTier(userId, tierDays) {
  const tier = STREAK_TIERS.find((t) => t.days === tierDays);
  if (!tier) return { ok: false, reason: "Palier invalide." };
  ...
  if (streak.streak < tier.days) {
    return {
      ok: false,
      reason: `Serie **${streak.streak}j** — palier **${tier.days}j** pas encore atteint.`,
    };
  }
  if (claims[String(tier.days)]) {
    return { ok: false, reason: `Palier **${tier.days}j** deja reclame.` };
  }

  claims[String(tier.days)] = Date.now();
  user.streakTierClaims = claims;
  store.save(data);
  const balance = economy.addCoins(userId, tier.bonus);
  return {
    ok: true,
    days: tier.days,
    bonus: tier.bonus,
...
function hasEconomyActivity(userId) {
  const data = ensureData();
  const u = data.users[userId];
  if (!u) return false;
  if ((u.stats?.casinoGames || 0) > 0) return true;
  if (u.questMeta?.lastClaimDay) return true;
  if ((u.quest?.progress || 0) > 0) return true;
  if (u.quest?.claimed) return true;
  return false;
}
