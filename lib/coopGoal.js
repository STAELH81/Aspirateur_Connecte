...
function reconcileGoalLock(data) {
  ...
  if (claimIds.length) {
    for (const userId of claimIds) {
      const removed = economy.removeCoins(userId, REWARD_COINS);
      if (!removed.ok) {
        console.warn(`[coop-goal] claim annule ${userId} — solde insuffisant pour -${REWARD_COINS}`);
      }
    }
    data.claims = {};
    changed = true;
...
async function announceGoalMet(client, data) {
  ...
  const { getMoneyHubChannelId } = require("./gamblingChannel");
  const questsChannelId =
    process.env.QUESTS_BOARD_CHANNEL_ID?.trim() || getMoneyHubChannelId();
