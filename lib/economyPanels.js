...
function infosPanelEmbed() {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        "Salons dedies : **money**, **casino**, **shop**, **infos**, **quetes**.",
        "Panneaux fixes = hub ; tes actions repondent en **message ephemere bleu**.",
        "Stats live : **aspirateurconnecte.netlify.app**",
      ].join("\n")
    )
    .addFields(
      {
        name: "Commandes & panneaux",
        value: [
          "`/casino` — flux casino (ephemere).",
          "**Money** — Daily, Work, Balance, Pay, Banque, Profil, top 25.",
          "**Shop** — achats par boutons + confirmation.",
          "**Quetes** — quete du jour, coop, suivi membres, **Claim quete** / **Claim coop**.",
          "**Infos** — ce guide (embed fixe).",
          "Staff : `/money admin` (panel, shop-panel, infos-panel, casino-panel, donner, retirer).",
          "Staff : `/quests panel|refresh` — poster / maj panneau quetes.",
          "Staff : `/gazette test|preview` · `/dashboard sync` · `/botstatus`.",
        ].join("\n"),
      },
      {
        ...
        value: [
          ...
          `**Banque** : pret **${c.bankLoan.minAmount}–${c.bankLoan.maxAmount}** coins (+**${Math.round(c.bankLoan.interestRate * 100)}%**), pour jouer au casino. **Ban casino** si pret en retard jusqu'au remboursement total.`,
        ].join("\n"),
      },
      {
        name: "Mises & jackpot",
      ...
      {
        name: "Quete du jour (panneau Quetes)",
        value: [
          "Chaque jour : une quete aleatoire (Daily, Work, casino…).",
          "Progression **automatique** quand tu fais l'action.",
          "Tableau 🟢🟠🔴 par membre — 🟢 = reclamee, 🟠 = faite / a claim, 🔴 = pas faite.",
          "Bouton **Claim quete** quand c'est termine (+ **serie** : bonus jusqu'a **+21** coins).",
          "Bouton **Ma quete** — ton suivi perso (quete + coop + serie).",
          "Rappel **DM 20h** si terminee mais pas reclamee.",
        ].join("\n"),
      },
      {
        name: "Objectif commu / coop (panneau Quetes)",
        value: [
          "Si le serveur cumule **30** parties casino dans la journee → bonus **+25** coins.",
          "Tu dois avoir joue **avant** que le cap soit atteint (pas les arrivants apres).",
          "Annonce auto dans le salon **casino** quand l'objectif tombe.",
          "Bouton **Claim coop** sur le panneau quetes — colonne Coop au 🟢 quand pris.",
          "Barre coop + site Netlify mis a jour via `/dashboard sync` (staff).",
        ].join("\n"),
      },
      {
        name: "Profil, site & Gazette",
        value: [
          "**Profil/Stats** (panneau Money ou `/userinfo`) : une ligne Quete + Coop, detail dans le tableau quetes.",
          "**aspirateurconnecte.netlify.app** — top coins, XP, casino du jour, graphiques coop.",
          "**La Gazette Du Gamblinnnnngggg** — recap casino chaque soir **23h59** (salon casino).",
        ].join("\n"),
      },
    );
}
