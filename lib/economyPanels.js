...
async function editCasinoUiMessage(interaction, body) {
  ...
  if (interaction.message?.editable) {
    await interaction.message.edit(body);
    trackCasinoUi(interaction.user.id, interaction.message);
    return interaction.message;
  }

  if (interaction.deferred || interaction.replied) {
    const msg = await interaction.editReply(body).catch(() => null);
    if (msg) trackCasinoUi(interaction.user.id, msg);
    return msg;
  }
...
async function publishCasinoResult(interaction, round) {
  ...
  const payload = round.payload || buildCasinoRoundUiPayload(interaction, round);
