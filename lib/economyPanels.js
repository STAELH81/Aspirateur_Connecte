...
async function handleCasinoConfigCancel(interaction) {
  ...
  await interaction.deferUpdate();
  await editTrackedMessage(interaction.client, casinoEphemeralUi, interaction.user.id, {
    embeds: [casinoPanelEmbed()],
...
async function applyCasinoRoundToUi(interaction, round) {
  const userId = interaction.user.id;
  if (!round.ok) {
    await editTrackedMessage(interaction.client, casinoEphemeralUi, userId, {
  ...
  if (round.blackjack) {
    const gameData = blackjack.getGame(round.blackjack.gameId, userId);
    await editTrackedMessage(interaction.client, casinoEphemeralUi, userId, {
  ...
  await editTrackedMessage(interaction.client, casinoEphemeralUi, userId, round.payload);
}
...
async function handleCasinoModalSubmit(interaction) {
  ...
  await applyCasinoRoundToUi(interaction, round);
  await ackModalSilently(interaction);
}
...
function describeShopItem(item) {
  ...
  switch (item.type || "role") {
    case "role":
      return `${price} — Role **${item.label}** pendant **${item.durationDays || 1}** jour(s). Retire automatiquement a expiration.`;
    case "daily_boost":
