...
const payLimits = require("./payLimits");
const { buildUserProfileEmbed } = require("./userProfile");
const { isMoneyHubChannel } = require("./gamblingChannel");
...
function isBankTopEligible(userId) {
  if (economy.isActiveAccount(economy.getAccount(userId))) return true;
  return gamblingProgress.hasEconomyActivity(userId);
}

function leaderboardLines(limit = Infinity, { activeOnly = false } = {}) {
  let rows = economy.getLeaderboard(Infinity);
  if (activeOnly) rows = rows.filter((e) => isBankTopEligible(e.id));
  if (Number.isFinite(limit)) rows = rows.slice(0, Math.max(0, Math.floor(limit)));
  return rows.map((e, i) => `${i + 1}. <@${e.id}> — **${e.balance}** coins`);
}

function buildTopEmbeds({ title = "Top coins", linesPerPage = 20, activeOnly = false } = {}) {
  const lines = leaderboardLines(Infinity, { activeOnly });
  if (lines.length === 0) {
    return [
      new EmbedBuilder()
        .setColor(COLOR_UI)
        .setTitle(title)
        .setDescription(activeOnly ? "Aucun compte actif pour l'instant." : "Personne n'a encore de coins."),
    ];
  }
...
function moneyPanelEmbed() {
  const top = leaderboardLines(25, { activeOnly: true });
  const topText =
    top.length === 0
  ...
  return new EmbedBuilder()
    .setColor(COLOR_UI)
    .setTitle("Banque")
    .setDescription("Top coins (comptes actifs) · **Balance** · **Pay** · **Profil** · **Pret**")
    .addFields(
      ...
      { name: "Top coins — comptes actifs (25 max)", value: topText }
    );
}
...
function formatActionCooldownLine(label, waitMs) {
  if (!waitMs || waitMs <= 0) return `**${label}** : disponible maintenant.`;
  const ts = toUnix(waitMs);
  return `**${label}** : <t:${ts}:R> (a <t:${ts}:T>)`;
}

function buildCooldownWaitMessage(title, waitMs) {
  const ts = toUnix(waitMs);
  return [
    `Cooldown actif.`,
    `Prochain **${title}** : <t:${ts}:R> (a <t:${ts}:T>).`,
    "Le compteur se met a jour tout seul — reclique sur le bouton pour rafraichir.",
  ].join("\n");
}

async function showMoneyFeedback(interaction, description, title, { persistent = false } = {}) {
  const userId = interaction.user.id;
  const body = { embeds: [blueEmbed(description, title)], components: [] };

  await interaction.deferUpdate().catch(() => {});
  await refreshPanelAfterAction(interaction);

  const edited = await editTrackedMessage(interaction.client, moneyEphemeralUi, userId, body);
  if (edited) {
    if (!persistent) scheduleMoneyMessageDeletion(interaction.client, edited.channelId, edited.id);
    return edited;
  }
  ...
  if (!persistent) scheduleMoneyMessageDeletion(interaction.client, msg.channelId, msg.id);
  return msg;
}

async function showMoneyCooldown(interaction, waitMs, title) {
  return showMoneyFeedback(interaction, buildCooldownWaitMessage(title, waitMs), title);
}
...
async function postPublicCasinoResult(interaction, payload) {
  if (!interaction.channel?.isTextBased() || !payload?.embeds?.length) return;
...
async function ackCasinoInteraction(interaction) {
  if (interaction.message?.id) {
    trackCasinoUi(interaction.user.id, interaction.message);
  }
  if (interaction.deferred || interaction.replied) return;
  await interaction.deferUpdate();
}

async function editCasinoUiMessage(interaction, body) {
...
async function publishCasinoResult(interaction, round) {
  if (round.blackjack && !round.payload) return;
  const payload = buildCasinoRoundUiPayload(interaction, round);
  if (!payload?.embeds?.length) return;
  await postPublicCasinoResult(interaction, payload);
}

async function finishCasinoRound(interaction, round) {
  if (round.blackjack && !round.payload) {
    await applyCasinoRoundToUi(interaction, round);
    return;
  }
  await publishCasinoResult(interaction, round);
  await applyCasinoRoundToUi(interaction, round);
  coopGoal.afterCasinoRound(interaction.user.id, interaction.client);
}
...
async function refreshPanelAfterAction(interaction) {
  if (isMoneyHubChannel(interaction.channel)) {
    require("./questsBoard").requestBoardRefresh(interaction.client);
    return;
  }
...
async function handleHubDailyWork(interaction, action) {
  const userId = interaction.user.id;

  if (action === "daily") {
    const result = economy.tryDaily(userId);
    if (!result.ok) {
      await showMoneyCooldown(interaction, result.waitMs, "Daily");
      return;
    }
  ...
  if (action === "work") {
    ...
    if (!result.ok) {
      await showMoneyCooldown(interaction, result.waitMs, "Work");
      return;
    }
    await economyLog.logTx(interaction.client, {
      userId,
...
async function handleMoneyPanelButton(interaction) {
  ...
  if (action === "bank") {
    await showBankFlow(interaction);
    return;
  }
  if (action === "profile") {
    const user = interaction.user;
    const member = interaction.member;
    const embed = buildUserProfileEmbed(user, member, member);
    await interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
    return;
  }

  if (action === "daily" || action === "work") {
    await interaction.reply({
      content: "**Daily** et **Work** sont sur le panneau du salon **#money** (ex-quetes).",
      flags: MessageFlags.Ephemeral,
    });
    return;
  }
  ...
  if (action === "balance") {
    ...
    const lines = [
      `Solde : ${economy.formatCoins(bal)}`,
      formatActionCooldownLine("Daily", economy.getDailyWait(userId)),
      formatActionCooldownLine("Work", economy.getWorkWait(userId)),
    ];
    if (loan.hasDebt) {
      lines.push(
        `Dette banque : **${loan.owed}** coins (emprunte **${loan.principal}**).`,
        loan.overdue
          ? "Pret **en retard** — casino bloque jusqu'au remboursement total."
          : "Pret en cours — casino OK avant l'echeance."
      );
    }
    await showMoneyFeedback(interaction, lines.join("\n"), "Balance");
    return;
  }
}
...
function buildCasinoRoundUiPayload(interaction, round) {
  if (!round.ok) {
    return {
  ...
  if (round.blackjack) {
    ...
    const playerName = playerDisplayName(interaction.user);
    return {
      embeds: [blackjack.buildEmbed(gameData, { playerName })],
...
async function applyCasinoRoundToUi(interaction, round) {
  return editCasinoUiMessage(interaction, buildCasinoRoundUiPayload(interaction, round));
}
...
async function handleCasinoSameBet(interaction) {
  ...
  await ackCasinoInteraction(interaction);
  ...
  await finishCasinoRound(interaction, round);
}
...
function infosPanelEmbed() {
  const c = economyCfg;
  const coopGoalGames = coopGoal.GOAL_GAMES;
  const milestone80 = Math.floor(coopGoalGames * 0.8);

  if (profile.isRockAndRoll()) {
    const c = economyCfg;
    return new EmbedBuilder()
      .setColor(COLOR_UI)
      .setTitle(`Guide — ${profile.brandName()}`)
      .setDescription(
        [
          "Tutoriel rapide pour ceux qui decouvrent l'economie du serveur.",
          "**Salons** : **Banque** · **Money** · **Casino** · **Infos** (ce panneau).",
          "Quand tu cliques sur un bouton, le bot te repond en **message bleu ephemere** (~5 min) — c'est normal.",
        ].join("\n")
      )
      .addFields(
        {
          name: "1 · Par ou commencer ?",
          value: [
            "**Roles** — salon dedie ou staff a poste le panel : boutons **CS2** · **Clips** · **Valo**.",
            "**Money** — bouton **Daily** (1× / 24 h) puis **Work** (toutes les ~45 min).",
            "**Money** — bouton **Quetes du jour** : une mission / jour, recompense en coins.",
            "**Banque** — **Balance** / **Profil** pour voir ton solde et ton XP.",
            "Ensuite : **Casino** si tu veux jouer (ou contribuer au **coop** du serveur).",
          ].join("\n"),
        },
        {
          name: "2 · Comment ca marche",
          value: [
            `Tu commences avec **${c.startBalance}** coins. Pas de shop ici — tout passe par les panneaux.`,
            `**Daily** : **${c.daily.min}–${c.daily.max}** coins · **Work** : **${c.work.min}–${c.work.max}** coins.`,
            "**Pay** (Banque) : envoyer des coins a un autre membre (limites anti-abus).",
            "**Pret** (Banque) : emprunter — rembourser sinon ban temporaire du casino.",
            "**/level voir** · **/level top** — XP en parlant sur le serveur (level up = message du bot).",
          ].join("\n"),
        },
        {
          name: "3 · Les panneaux",
          value: [
            "**Banque** — top comptes **actifs**, balance, pay, profil, pret.",
            "**Money** — daily, work, quetes, coop du jour, tableau, paliers serie.",
            "**Casino** — jeux (coinflip, slots, des…), jackpot, **duels** entre joueurs.",
            "**Infos** — ce guide (staff : `/money admin infos-panel` pour le reposter).",
          ].join("\n"),
        },
        {
          name: "4 · Quetes & serie",
          value: [
            "1 quete / jour (faire un daily, jouer au casino, etc.) — progression **auto**.",
            "Claim sur le panneau **Money** quand c'est fini.",
            "**Serie** : claim ta quete plusieurs jours d'affilee → bonus au claim (+3/j dès J2).",
            "Bouton **Paliers serie** sur Money : voir ta serie et claim les bonus (**7j** / **14j** / **21j**).",
          ].join("\n"),
        },
        {
          name: "5 · Coop & duels",
          value: [
            `Objectif serveur : **${coopGoalGames}** parties casino / jour → **+25** coins si tu as joue **avant** le cap.`,
            "Claim sur **Money** quand la barre est pleine — pas apres (trop tard).",
            `**Duels** (/duel ou panneau casino) : manche = 1 partie coop · max **${coopGoal.DUEL_COOP_MAX_ROUNDS_PER_DUEL}**/duel · mise min **${coopGoal.DUEL_COOP_MIN_BET}**.`,
          ].join("\n"),
        },
        {
          name: "6 · Autre",
          value: [
            "**Radio** — salon vocal dedie : le bot shuffle les sons en boucle.",
            "**/afk on|off** · **/suggest idee** · **/ticket** (panel staff).",
            "**/help** — liste des commandes · @BotQuick si tu as une question.",
          ].join("\n"),
        }
      )
      .setFooter({ text: `${profile.footerText()} · panneau Infos` });
  }

  return new EmbedBuilder()
    .setColor(COLOR_UI)
    .setTitle(`Guide — ${profile.brandName()} (v1.2.10)`)
    .setDescription(
      [
        "**Salons** : **Banque** · **Money** · Casino · Shop · Infos",
        "Panneaux = hub fixe · tes actions = **message ephemere bleu** (~5 min sur Money).",
        "Stats live : **aspirateurconnecte.netlify.app**",
      ].join("\n")
    )
    .addFields(
      {
        name: "Panneaux membres",
        value: [
          "**Banque** — top 25 **comptes actifs**, balance, pay, profil, **pret**",
          "**Money** — daily, work, quetes, coop, tableau (quetes·coop·daily·work)",
      ...
      },
      {
        ...
        value: [
          ...
          "Tableau maj auto ~**10 s** · rappel **DM 20h** si quete finie ou coop claimable.",
        ].join("\n"),
      },
      {
        name: "Quetes du jour",
        value: [
          "1 quete / jour (Daily, Work, casino…). Progression **auto**.",
          "Ex. : Daily **+35** · 2 Work **+45** · 3 parties **+50** · 1 win **+60** · 150 misés **+55**.",
          "**Serie** (claim quete) : **+3/jour** des le J2 (max **+21** bonus au claim quete).",
        ].join("\n"),
      },
      {
        name: "Paliers serie (bouton Money)",
        value: [
          "Bonus **one-shot** selon ta serie de claims quete :",
          "**7j** → **+50** · **14j** → **+120** · **21j** → **+250** coins.",
          "Bouton **Paliers serie** : voir ta serie + claim si eligible.",
        ].join("\n"),
      },
      {
        name: "Objectif commu (coop)",
        value: [
          `**${coopGoalGames}** parties casino serveur / jour → **+25** coins.`,
          "Tu dois avoir joue **avant** le cap — pas apres (⚫ = trop tard).",
          `Alerte **80 %** (**${milestone80}/${coopGoalGames}**) dans **casino** · annonce cap + claim sur **Money**.`,
          "**MVP** top 3 contributeurs : **+15 / +10 / +5** en plus au claim (ligne bonus = …).",
          `**Duels** : chaque manche compte (max **${coopGoal.DUEL_COOP_MAX_ROUNDS_PER_DUEL}**/duel) · mise min **${coopGoal.DUEL_COOP_MIN_BET}** coins.`,
        ].join("\n"),
      },
      {
        name: "Duels (/duel) — salon casino",
        value: [
          "/duel @joueur — **coinflip**, **slots** ou **de**.",
          "Objectif **1–100 victoires** · mise par manche · escrow **mise × objectif** des deux.",
          "Seul le joueur defie peut accepter (60 s) · gagnant prend le pot (−5 % jackpot).",
          `Compte dans le **coop** : chaque **manche** jouee (max **${coopGoal.DUEL_COOP_MAX_ROUNDS_PER_DUEL}**/duel) · mise min **${coopGoal.DUEL_COOP_MIN_BET}** coins.`,
        ].join("\n"),
      },
      {
        name: "Gagner des coins",
        value: [
          `**Daily** : ${c.daily.min}–${c.daily.max} · **24 h** · streak daily max **+${c.dailyStreak.maxBonus}**`,
          `**Work** : ${c.work.min}–${c.work.max} · **45 min**`,
          `**Pret** (Banque) : **${c.bankLoan.minAmount}–${c.bankLoan.maxAmount}** (+**${Math.round(c.bankLoan.interestRate * 100)}%**) · ban casino si retard`,
          `Depart : **${c.startBalance}** coins · Pay entre membres (plafonds anti-abus)`,
        ].join("\n"),
      },
      {
        name: "Casino & jackpot",
        value: [
          `\`/casino\` ou panneau Casino · mise min **${c.bet.min}** · max **${Math.round(c.bet.maxBalanceRatio * 100)}%** / **${c.bet.maxAbsolute}**`,
          `Coinflip x**${c.coinflip.multiplier}** · Dice x**${c.dice.multiplier}** · Roulette · Slots · Blackjack`,
          `Jackpot : **${Math.round(c.jackpot.taxRate * 100)}%** / mise · ~**1/${Math.round(1 / c.jackpot.winChance)}** chance de toucher`,
        ].join("\n"),
      },
      {
        name: "Profil, site & staff",
        value: [
          "**Profil/Stats** ou `/userinfo` : resume Quete · Coop → detail tableau Money.",
          "**Gazette** — recap casino **23h59**.",
          "Staff : `/money admin` (dont **fermer-parties** blackjack) · `/quests panel` · `/devlog post` · `/gazette` · `/dashboard sync` · `/botstatus`.",
        ].join("\n"),
      }
    );
}

function bankPanelRows(status) {
  const canBorrow = !status.hasDebt && status.cooldownMs === 0;
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("money:bank:borrow")
        .setLabel("Emprunter")
        .setStyle(ButtonStyle.Success)
        .setDisabled(!canBorrow),
      new ButtonBuilder()
        .setCustomId("money:bank:repay")
        .setLabel("Rembourser")
        .setStyle(ButtonStyle.Primary)
        .setDisabled(!status.hasDebt)
    ),
  ];
}

async function showBankFlow(interaction) {
  const status = bankLoans.getStatus(interaction.user.id);
  await interaction.reply({
    embeds: [blueEmbed(bankLoans.formatStatusLines(status).join("\n"), "Banque")],
    components: bankPanelRows(status),
    flags: MessageFlags.Ephemeral,
  });
}

function buildBankBorrowModal() {
  const c = bankLoans.loanCfg();
  return new ModalBuilder()
    .setCustomId("money:bank:modal:borrow")
    .setTitle("Pret banque — emprunter")
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId("montant")
          .setLabel(`Montant (${c.minAmount}–${c.maxAmount})`)
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setPlaceholder(String(c.minAmount))
      )
    );
}

function buildBankRepayModal(owed) {
  return new ModalBuilder()
    .setCustomId("money:bank:modal:repay")
    .setTitle("Pret banque — rembourser")
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId("montant")
          .setLabel("Montant (partiel ou total)")
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setPlaceholder(String(owed))
          .setValue(String(owed))
      )
    );
}

async function handleBankOpenModal(interaction) {
  const action = interaction.customId.split(":")[2];
  const status = bankLoans.getStatus(interaction.user.id);
  if (action === "borrow") {
    if (status.hasDebt) {
      await interaction.reply({
        content: "Tu as deja un pret en cours.",
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    if (status.cooldownMs > 0) {
      const ts = toUnix(status.cooldownMs);
      await interaction.reply({
        content: `Prochain pret <t:${ts}:R>.`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    await interaction.showModal(buildBankBorrowModal());
    return;
  }
  if (action === "repay") {
    if (!status.hasDebt) {
      await interaction.reply({
        content: "Aucune dette a rembourser.",
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    await interaction.showModal(buildBankRepayModal(status.owed));
  }
}

async function handleBankBorrowModal(interaction) {
  const amount = parseInt(String(interaction.fields.getTextInputValue("montant") || "").trim(), 10);
  if (!Number.isFinite(amount)) {
    await interaction.reply({ content: "Montant invalide.", flags: MessageFlags.Ephemeral });
    return;
  }

  const result = bankLoans.takeLoan(interaction.user.id, amount);
  if (!result.ok) {
    if (result.waitMs) {
      const ts = toUnix(result.waitMs);
      await interaction.reply({
        content: `Pret indisponible. Reviens <t:${ts}:R> (a <t:${ts}:T>).`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
  ...
  await economyLog.logTx(interaction.client, {
    ...
    balanceAfter: result.balanceAfter,
    details: [
      `Emprunte : **+${result.principal}** coins`,
      `A rembourser : **${result.owed}** coins (interet **${result.interestPercent}%**)`,
      result.dueAt
        ? `Echeance : <t:${Math.floor(result.dueAt / 1000)}:f>`
        : null,
    ]
      .filter(Boolean)
      .join("\n"),
  ...
  await interaction.reply({
    embeds: [
      blueEmbed(
        [
          `Pret accorde : **+${result.principal}** coins.`,
          `Dette totale : **${result.owed}** coins (interet **${result.interestPercent}%**).`,
          `Solde : ${economy.formatCoins(result.balance)}`,
          `Rembourse avant <t:${Math.floor(result.dueAt / 1000)}:R> pour eviter le ban casino.`,
          "Apres l'echeance : casino bloque jusqu'au remboursement **total**.",
          result.dueAt
            ? `Echeance indicative : <t:${Math.floor(result.dueAt / 1000)}:R>.`
            : null,
        ]
          .filter(Boolean)
          .join("\n"),
...
async function handleBankRepayModal(interaction) {
  const amount = parseInt(String(interaction.fields.getTextInputValue("montant") || "").trim(), 10);
  if (!Number.isFinite(amount)) {
    await interaction.reply({ content: "Montant invalide.", flags: MessageFlags.Ephemeral });
    return;
  ...
  const result = bankLoans.repayLoan(interaction.user.id, amount);
  if (!result.ok) {
    await interaction.reply({ content: result.reason, flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  await economyLog.logTx(interaction.client, {
    userId: interaction.user.id,
    action: "Remboursement banque",
    balanceBefore: result.balanceBefore,
    balanceAfter: result.balanceAfter,
    details: [
      `Verse : **-${result.paid}** coins`,
      result.cleared
        ? "Pret **solde**."
        : `Reste a payer : **${result.remaining}** coins`,
    ].join("\n"),
  });

  const lines = [
    `Rembourse : **${result.paid}** coins.`,
    result.cleared
      ? "Dette **terminee** — casino debloque si tu etais en retard."
      : `Reste : **${result.remaining}** coins.`,
    `Solde : ${economy.formatCoins(result.balance)}`,
  ];
  await interaction.reply({
    embeds: [blueEmbed(lines.join("\n"), "Banque")],
    flags: MessageFlags.Ephemeral,
  });
}
...
async function handlePayConfirm(interaction) {
  ...
  const limitCheck = payLimits.checkPay(interaction.user.id, draft.toId, draft.amount);
  if (!limitCheck.ok) {
    await interaction.update({ content: limitCheck.reason, embeds: [], components: [] });
    return;
  }
  ...
  payLimits.recordPay(interaction.user.id, draft.toId, result.amount);
  ...
  if (payLimits.shouldAlert(result.amount)) {
    await economyLog
      .logEvent(interaction.client, {
        title: "Pay — gros transfert",
        lines: [
          `De : <@${interaction.user.id}>`,
          `Vers : <@${draft.toId}>`,
          `Montant : **${result.amount}** coins`,
