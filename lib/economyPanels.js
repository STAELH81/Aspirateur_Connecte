...
async function updateCasinoUi(interaction, { embeds, components }) {
  ...
  if (ref) {
    try {
      ...
      const msg = await channel.messages.fetch(ref.messageId);
      await msg.edit({ embeds, components: components || [] });
  ...
  casinoEphemeralUi.set(userId, { channelId: msg.channelId, messageId: msg.id });
  return msg;
}
...
async function runCasinoRound(interaction, { game, bet, choice, number }) {
  ...
  if (game === "blackjack") {
    const result = blackjack.startGame(userId, mise);
    if (!result.ok) return result;
    if (result.instant) {
...
async function postCasinoResult(interaction, payload) {
  ...
  const { content: _drop, ...rest } = payload;
  const msg = await interaction.channel.send(rest);
...
async function openCasinoGameConfig(interaction, game) {
  ...
  const ref = casinoEphemeralUi.get(interaction.user.id);
  if (ref?.messageId === interaction.message?.id) {
    return updateCasinoUi(interaction, payload);
  }
  await deleteCasinoEphemeralUi(interaction.client, interaction.user.id);
  casinoEphemeralUi.delete(interaction.user.id);
...
async function postMoneyActionInChannel(interaction, action, messagePayload) {
  ...
  if (!payload.embeds?.length && payload.content) {
    payload.embeds = [blueEmbed(payload.content)];
...
async function handleMoneyPanelButton(interaction) {
  ...
  if (action === "balance") {
    ...
    await postMoneyActionInChannel(interaction, "balance", {
      embeds: [blueEmbed(`${interaction.user} : ${economy.formatCoins(bal)}`, "Balance")],
    });
    return;
  }
  ...
  if (action === "work") {
    ...
    if (!result.ok) {
      ...
      await postMoneyActionInChannel(interaction, "work", {
        embeds: [
          blueEmbed(`${interaction.user} — Repos. Retente <t:${ts}:R> (a <t:${ts}:T>).`, "Work"),
        ],
      });
      return;
    }
...
async function handleCasinoOtherGame(interaction) {
  ...
  if (!interaction.replied && !interaction.deferred) {
    await interaction.deferUpdate().catch(() => interaction.deferReply({ flags: MessageFlags.Ephemeral }));
  }
  await showCasinoGamePicker(interaction, "casino:pick:select");
}
...
async function handleCasinoSameBet(interaction) {
  ...
  if (!round.ok) {
    await interaction.editReply({ content: round.reason || "Partie impossible." });
    return;
  }
  if (round.blackjack) {
    await deleteCasinoEphemeralUi(interaction.client, interaction.user.id);
  ...
  await postCasinoResult(interaction, round.payload);
}
...
async function handleCasinoReplay(interaction) {
  ...
  await updateCasinoUi(interaction, {
    embeds: [casinoConfigEmbed(state, interaction.user)],
    components: casinoConfigRows(state, interaction.user.id),
  });
}
...
function shopPanelEmbed() {
  ...
  const lines =
    items.length === 0
      ...
      : items.map((item, i) => `**${i + 1}. ${item.label}** — ${item.price} coins`).join("\n");

  return blueEmbed(
    ["Clique sur un bouton pour acheter un article.", "", lines].join("\n"),
    "Shop"
  );
}
...
function infosPanelEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        "Module economie & casino du serveur.",
        "",
        "**Money** — Daily, Work, Balance, Pay. Le top 25 est visible sur le panneau (mis a jour auto).",
        "**Shop** — achete des bonus / roles avec les boutons.",
        "**Casino** — `/casino` : choisis un jeu, configure, lance ta mise.",
        "",
        "Apres une partie : **Meme mise** (rejoue pareil), **Rejouer** (change la mise), **Changer de jeux**.",
        "",
        "Jackpot commun · resultats casino supprimes apres **1 heure**.",
      ].join("\n")
    );
}
...
async function handlePayConfirm(interaction) {
  ...
  scheduleMoneyMessageDeletion(interaction.client, payMsg.channelId, payMsg.id);
}
...
async function handleShopConfirmButton(interaction) {
  ...
  const shopMsg = await interaction.followUp({
    embeds: [
      blueEmbed(
