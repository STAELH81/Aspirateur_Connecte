...
function scheduleCasinoEphemeralCleanup(client, userId, delayMs = 10_000) {
  setTimeout(() => {
    deleteCasinoEphemeralUi(client, userId).catch(() => {});
  }, delayMs);
}
...
async function postCasinoResult(interaction, payload) {
  scheduleCasinoEphemeralCleanup(interaction.client, interaction.user.id, 10_000);
...
async function handleCasinoConfigCancel(interaction) {
  ...
  scheduleCasinoEphemeralCleanup(interaction.client, interaction.user.id, 10_000);
  await interaction.update({
    embeds: [blueEmbed("Configuration annulee.", "Casino")],
    components: [],
  });
}
...
async function handleCasinoOtherGame(interaction) {
  casinoConfigState.delete(interaction.user.id);
  if (!casinoEphemeralUi.has(interaction.user.id) && !interaction.replied && !interaction.deferred) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  }
  await showCasinoGamePicker(interaction);
}
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "",
        "Jackpot commun · resultats casino supprimes apres **2 jours**.",
      ].join("\n")
    );
}
