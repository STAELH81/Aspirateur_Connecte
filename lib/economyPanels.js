...
function moneyPanelEmbed() {
  ...
  const topText =
    top.length === 0
      ...
      : top.map((e, i) => `${i + 1}. <@${e.id}> — **${e.balance}** coins`).join("\n");
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "Le top se met a jour automatiquement a chaque interaction.",
      ].join("\n")
    )
    .addFields(
...
function buildCasinoModal(game) {
  const modal = new ModalBuilder()
    .setCustomId(`casino:play:${game}`)
...
async function handleCasinoModalSubmit(interaction) {
  ...
  if (game === "coinflip") {
    const coteRaw = String(interaction.fields.getTextInputValue("cote") || "")
      .trim()
      .toLowerCase();
