...
function moneyPanelEmbed() {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        "Salon **#bank** — solde, pay, profil, emprunts.",
        "Daily · Work · quetes · coop → panneau **#money**.",
        "Stats : **aspirateurconnecte.netlify.app**",
      ].join("\n")
    )
    .addFields(
...
async function handleMoneyPanelButton(interaction) {
  ...
  if (action === "daily" || action === "work") {
    await interaction.reply({
      content: "**Daily** et **Work** sont sur le panneau **#money** (ex-quetes).",
      flags: MessageFlags.Ephemeral,
    });
    return;
  }
...
function infosPanelEmbed() {
  ...
  return new EmbedBuilder()
    .setColor(COLOR_UI)
    .setTitle("Guide gambling — Les Girlsss")
    .setDescription(
      [
        "**Salons** : Money · Casino · Shop · Infos · **Quetes**",
        "Panneaux = hub fixe · tes actions = **message ephemere bleu**.",
    ...
    .addFields(
      {
        ...
        value: [
          "**Money** — Daily, Work, Balance, Pay, Banque, Profil, Refresh, top 25",
          "**Casino** — jeux, jackpot, Meme mise / Rejouer / Changer de jeux",
          "**Shop** — achats par boutons + confirmation",
          "**Quetes** — hub quetes & coop (voir ci-dessous)",
          "**Infos** — ce guide",
        ].join("\n"),
      },
      {
        name: "Quetes Center (v1.2.8.5)",
        value: [
          "Tableau 🟢🟠🔴⚫ par membre (Quete + Coop).",
          "**Quetes du jour** — réclamer la quête terminée (+ série bonus).",
          "**Coop du jour** — réclamer le bonus commu si éligible.",
          "**Infos quotidiennes** — ton suivi (quete, coop, serie).",
          "**Refresh** — maj tableau · auto ~**10 s**.",
          "Rappel **DM 20h** si quete finie ou coop claimable.",
        ].join("\n"),
      },
      {
        name: "Quetes du jour",
        value: [
          ...
          "**Serie** : claim plusieurs jours d'affilee → **+3/jour** des le J2 (max **+21** bonus).",
        ].join("\n"),
      },
      {
        name: "Objectif commu (coop)",
        value: [
          `**${coopGoalGames}** parties casino sur le serveur / jour → **+25** coins.`,
          "Tu dois avoir joue **avant** le cap — pas apres (⚫ = trop tard).",
          "Annonce dans **casino** quand c'est atteint · claim sur panneau **Quetes**.",
        ].join("\n"),
      },
      {
        ...
        value: [
          ...
          `**Banque** (Money) : pret **${c.bankLoan.minAmount}–${c.bankLoan.maxAmount}** (+**${Math.round(c.bankLoan.interestRate * 100)}%**) · ban casino si retard`,
      ...
      {
        name: "Profil, site & auto",
        value: [
          "**Profil/Stats** ou `/userinfo` : resume Quete · Coop → detail tableau quetes.",
          "**Gazette** — recap casino **23h59** (salon casino).",
          "Staff : `/money admin` · `/quests panel` · `/gazette` · `/dashboard sync` · `/botstatus`.",
        ].join("\n"),
      }
    );
}
