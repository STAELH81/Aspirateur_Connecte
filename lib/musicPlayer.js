...
const MIN_TRACK_MS = 1500;
const FAST_SKIP_DELAY_MS = 3000;
...
function stopRadio() {
  if (!radioSession) return;
  radioSession.active = false;
  const { player, connection, client, channelId } = radioSession;
  radioSession = null;
  ...
  if (client && channelId) {
    clearNowPlaying(client, channelId).catch(() => null);
  }
}
...
function buildRadioController(ctx) {
  const { client, channel, player, allTracks, session } = ctx;
  ...
  async function playNext() {
    if (!session.active) return { ok: false, message: "Radio arretee." };
    if (session.connection.state.status === VoiceConnectionStatus.Destroyed) {
      return { ok: false, message: "Connexion vocale detruite." };
    }
    if (advancing) return { ok: false, message: "Lecture deja en cours." };
...
async function startMusic(client, { force = false } = {}) {
  ...
  if (startingMusic) {
    console.log("[music] Demarrage deja en cours, ignore.");
    return;
  }
  ...
  try {
    ...
    player.on("stateChange", (oldState, newState) => {
      if (!session.active || radioSession !== session) return;
      console.log(`[music] Player ${oldState.status} -> ${newState.status}`);
      if (
        oldState.status === AudioPlayerStatus.Playing &&
        newState.status === AudioPlayerStatus.Idle
      ) {
        const playedMs = Date.now() - session.trackStartedAt;
        if (playedMs < MIN_TRACK_MS) {
          console.warn(
            `[music] Piste coupee trop vite (${playedMs}ms) — pause ${FAST_SKIP_DELAY_MS / 1000}s`
          );
          setTimeout(
    ...
    const started = Boolean(first?.ok);
