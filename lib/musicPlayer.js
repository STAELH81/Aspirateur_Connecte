...
async function startMusic(client) {
  ...
  if (!channelId || !guildId) {
    console.log("[music] MUSIC_VOICE_CHANNEL_ID non defini.");
    return;
  }

  const tracks = loadTracks();
  if (!tracks.length) {
    console.log("[music] Aucune piste dans data/music/ ou music-playlist.json");
    return;
  }
  ...
  if (!channel?.isVoiceBased()) {
    console.warn("[music] Salon vocal invalide.");
    return;
  }
