...
async function buildGazetteEmbed(guild, recap) {
  ...
  return new EmbedBuilder()
    ...
    .setTimestamp(new Date(`${key}T23:59:00`));
}

async function postGazetteForDay(client, dayKey, options = {}) {
  const channelId =
    options.channelId ||
  ...
  if (!channelId || !guildId) {
    return { ok: false, reason: "GAMBLING_CHANNEL_ID ou DISCORD_GUILD_ID manquant." };
  }

  const key = dayKey || todayKey();
  if (!options.force) {
    const state = loadState();
    if (state.lastPostedDayKey === key) {
      return { ok: false, reason: `Gazette deja publiee pour **${key}**. Utilise \`test\` pour forcer.` };
    }
  }

  const recap = gamblingProgress.getDailyRecap(key);
  const guild = await client.guilds.fetch(guildId).catch(() => null);
  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!guild || !channel?.isTextBased()) {
    return { ok: false, reason: "Salon gambling ou serveur introuvable." };
  }

  const embed = await buildGazetteEmbed(guild, recap);
  if (options.ephemeralReply) {
    return { ok: true, embed, recap, preview: true };
  }

  await channel.send({
    content: options.test ? "*(test staff — gazette)*" : null,
    embeds: [embed],
  });

  if (!options.skipState && !options.test) {
    const state = loadState();
    state.lastPostedDayKey = key;
    saveState(state);
  }

  return { ok: true, recap, channelId };
}

async function postGazette(client) {
  return postGazetteForDay(client, todayKey(), { force: false });
}
