...
const WINS_NEEDED = { 1: 1, 3: 2, 5: 3 };

function genId() {
  return crypto.randomBytes(4).toString("hex");
}

function winsNeeded(rounds) {
  return WINS_NEEDED[rounds] || Math.ceil(rounds / 2);
}
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        `Manches : **${duel.rounds}** (premier a **${needed}** victoire(s))`,
        `Mise : **${duel.bet}** coins / manche`,
        `Escrow : **${duel.escrow}** coins chacun au accept`,
        "",
...
function buildActiveEmbed(duel, extraLines = []) {
  ...
  const lines = [
    ...
    `Score : **${duel.scoreA}** — **${duel.scoreB}** (BO${duel.rounds})`,
    `Pot : **${duel.pot}** coins`,
  ];
  if (duel.currentRound) lines.push(`Manche **${duel.currentRound}** / ${duel.rounds}`);
...
async function resolveRound(client, duel) {
  ...
  if (duel.currentRound >= duel.rounds && duel.scoreA === duel.scoreB) {
    duel.currentRound += 1;
  } else if (duel.currentRound < duel.rounds) {
    duel.currentRound += 1;
  } else {
    const winSlot = duel.scoreA > duel.scoreB ? "a" : "b";
    saveDuel(duel);
    await finishDuel(client, duel, winSlot, lines);
    return;
  }
...
async function handleSetupModal(interaction) {
  ...
  if (![1, 3, 5].includes(rounds)) {
    await interaction.reply({
      content: "Manches invalides — choisis **1**, **3** ou **5**.",
      flags: MessageFlags.Ephemeral,
    });
    return;
  }
