...
function categoryValidationError(channel, categoryId) {
  if (!channel) {
    return (
      ...
      "(Discloud aussi) et que le bot est bien sur Les Girlsss."
    );
  }
...
function formatOpenError(err, categoryId) {
  ...
  if (err.code === 50035) {
    return (
      `L'ID **${categoryId}** dans TICKET_CATEGORY_ID n'est **pas** une categorie (Discord: salon texte ou autre). ` +
      "Sur Discloud **et** en local : clic droit sur le **titre** du dossier Tickets → Copier l'ID de la categorie. " +
      "Ou poste `/ticket panel` dans un salon **deja dans** la categorie Tickets (auto)."
    );
  }
...
async function openTicket(interaction) {
  ...
  try {
    ...
    return { ok: false, reason: formatOpenError(err, categoryId) };
  }
