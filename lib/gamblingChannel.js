...
function checkGamblingChannel(interaction) {
  ...
  return {
    ok: false,
    message: `L'economie et le casino sont uniquement dans ${mentions} (ou salons nommes bank/money/casino/shop/infos).`,
  };
}
...
function isMoneyHubChannel(channel) {
  ...
  if (configured && channel.id === configured) return true;
