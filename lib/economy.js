...
/** Compte ayant bouge dans l'economie (solde, daily/work, pret). */
function isActiveAccount(acc) {
  if (!acc) return false;
  if (acc.balance !== cfg.startBalance) return true;
  if (acc.lastDaily > 0 || acc.lastWork > 0) return true;
  if (acc.lastDailyDay) return true;
  if (acc.bankLoan) return true;
  return false;
}
...
function hasDailyToday(userId) {
  const acc = getAccount(userId);
  return acc.lastDailyDay === dayKey();
}

function hasWorkToday(userId) {
  const acc = getAccount(userId);
  if (!acc.lastWork) return false;
  return dayKey(acc.lastWork) === dayKey();
}
