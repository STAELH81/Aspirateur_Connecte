...
function tryDaily(userId) {
  ...
  return { ok: true, gain, balance: acc.balance, streak, bonus, boostUsed, boostMult };
}
...
function tryWork(userId) {
  ...
  data[userId] = acc;
  save(data);
  return { ok: true, gain, balance: acc.balance, workBoostUsed, workMult };
}
...
function adminGrant(targetId, amount) {
  ...
  return { ok: true, amount: n, balance };
}
...
function adminRemove(targetId, amount) {
  ...
  if (!removed.ok) return { ok: false, reason: "Solde insuffisant." };
  return { ok: true, amount: n, balance: removed.balance };
}
